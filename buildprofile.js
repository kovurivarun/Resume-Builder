function handleLoad() {
    ...
    let PerInfoAcc = document.getElementById("perinfoaccbtn")

    const formatDateForInput = (d) => {
        return `${d.getFullYear()}-${(d.getMonth() + 1).toString().padStart(2, "0")}-${(d.getDate()).toString().padStart(2, "0")}`
    }

    // sets the personal info values.
    const setPersonalInfo = (personalInfo) => {
        console.log({ personalInfo });

        const { DOB, email, firstname, lastname, address } = personalInfo;
        ...
        document.getElementById('dateOfBirth').value = formatDateForInput(dateOfBirth);
    ...
    const fetchPersonalInfo = () => {
        const { APPLICATION_JSON } = window?.config?.DEFAULT_HEADERS;

        window?.fetch(window?.config?.ApiUrls?.FETCHPERSONALINFO, {
            method: window?.config?.HTTP_METHODS?.GET,
    ...
    PerInfoAcc.addEventListener('click', fetchPersonalInfo)
